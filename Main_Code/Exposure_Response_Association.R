...
summary(mv)
concentration <- seq(bound[1],bound[2],length=30)
bConcentration <- onebasis(concentration,fun=type,degree=degree,knots=knots,Bound=bound)

(mvreml <- mvmeta(ymat,Slist,method="reml"))
cpreml <- crosspred(bConcentration,coef=coef(mvreml),vcov=vcov(mvreml),
  model.link="log",by=1,cen=cen)
cp <- crosspred(bConcentration,coef=coef(mv),vcov=vcov(mv),model.link="log",
  by=1,cen=cen)
...
breaks <- c(min(data$concentration,na.rm=T)-1,seq(cp$predvar[1],
                                          cp$predvar[length(cp$predvar)],length=30),max(data$concentration,na.rm=T)+1)
hist <- hist(data$concentration,breaks=breaks,plot=F)
